    ...
    tortoises = [
        {
            ...
            "bomb_x": None,  # Tracks the bomb position
            "bomb_timer": None,  # Tracks the bomb timer
            "boom_frame": None  # Tracks the frame when BOOM should disappear
        }
    ...
    while not winner:
        ...
        for tortoise in tortoises:
            ...
            if tortoise["exploded"]:
                total_exploded += 1
                if tortoise["boom_frame"] and frame_counter - tortoise["boom_frame"] < 30:  # Display BOOM for 3 seconds (30 frames)
                    stdscr.addstr(tortoise["y"], tortoise["bomb_x"], BOOM, curses.A_BOLD)
                continue

            # Randomly place a bomb with a smaller chance (1% per frame)
            if random.random() < 0.01 and tortoise["bomb_x"] is None:
                tortoise["bomb_x"] = int(tortoise["x"]) + 12
                tortoise["bomb_timer"] = 3  # Reset bomb timer to 3 seconds

            # Handle bomb countdown (1 second every 10 frames)
            if tortoise["bomb_x"] is not None:
                if frame_counter % 10 == 0 and tortoise["bomb_timer"] > 0:  # Decrease every 1 second (10 frames)
                ...
                if tortoise["bomb_timer"] > 0:
                    ...
                    # Bomb explodes
                    tortoise["boom_frame"] = frame_counter
                    stdscr.addstr(tortoise["y"], tortoise["bomb_x"], BOOM, curses.A_BOLD)
                    tortoise["exploded"] = True
                    tortoise["bomb_x"] = None  # Reset bomb position
                    tortoise["bomb_timer"] = None  # Reset bomb timer
                    continue
